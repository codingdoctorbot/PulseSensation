...
local function persist()
    Pulse.Database:SetLocomotionProfile(
        character.race, character.ridingTier, character.armorMultiplier)
end
...
local function gaitWaveform(mode, phase, intensity, sharpness)
    ...
    if peak < 0 then peak = 0 elseif peak > 1 then peak = 1 end
...
local function tick(_, elapsed)
    ...
    if shouldSplitFeet() then
        ...
        Pulse:HoldRolesIfEnabled(CUE, { ltrigger = left, rtrigger = right }, 0.2)
    else
...
local function sync()
    ...
    refreshRace(); refreshRidingTier(); refreshArmorWeight(); persist()
    local speed = resolveSpeed()
    if speed > 0.05 then
        isMoving = true
        resolveGait()
...
function M:_DebugGait()
    return {
        race = character.race, ridingTier = character.ridingTier,
        armorMultiplier = character.armorMultiplier,
        mount = (RACE_GAITS[character.race] or FALLBACK_RACE).mount,
        mode = gait.mode, cadence = gait.cadence,
        intensity = gait.intensity, sharpness = gait.sharpness,
        valid = gait.valid, inCombat = inCombat, lastGoodSpeed = lastGoodSpeed,
