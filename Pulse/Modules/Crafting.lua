...
local PROFESSION_ORDER = {
    BLACKSMITHING, MINING, ENGINEERING, JEWELCRAFTING, LEATHERWORKING,
    TAILORING, ALCHEMY, ENCHANTING, INSCRIPTION, COOKING, FIRSTAID,
...
local function resolveProfession(recipeSpellID)
    ...
    if type(info.profession) ~= "number" then return nil end
...
local function beginCraft(recipeSpellID)
    ...
    if Pulse.debug then
        print(("Pulse: craft started — %s (recipe %s)")
            :format(work.label, tostring(recipeSpellID)))
    end
end
...
function M:_PlayStrike()
    if not work.mode then return end
...
local function tick()
    ...
    if bed > 0 then
        Pulse:HoldRolesIfEnabled(CUE, { low = clamp01(bed) })
    end

    if not work.mode or work.cadence <= 0 then return end
...
function M:_DebugCraft()
    return {
        active = active, profession = work.label, bed = work.bed,
        cadence = work.cadence, mode = work.mode, strike = work.strike,
        gain = gain, strikesTotal = strikesTotal, nextStrike = nextStrike,
    }
end
