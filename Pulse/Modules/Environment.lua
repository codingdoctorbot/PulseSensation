...
local function startBreathTicker(maxValue)
    ...
    breathTicker = C_Timer.NewTicker(0.1, function()
        ...
        if not current or current <= 0 then
            -- If still submerged, transition into drowning!
            if submerged and (warnedLowBreath or (breathMaxValue and breathMaxValue > 0)) then
                isDrowning = true
...
breathFrame:SetScript("OnEvent", function(_, event, timerName, _, maxValue)
    ...
    elseif event == "MIRROR_TIMER_STOP" then
        if isPlayerSubmerged() and (warnedLowBreath or (breathMaxValue and breathMaxValue > 0)) then
