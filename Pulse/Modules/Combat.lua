...
local function GetActiveWeaponSpeeds()
    ...
    --
    -- STILL WRONG, left deliberately: the constant below is a real weapon speed but is
    -- divided by `mult` a few lines down as though it were an unhasted base, so on a hasted
    -- character it scales the wrong way. Fixing it retunes weaponSwingMain, a cue that is
    -- off by default and already documented as an estimate that desyncs, so it belongs in a
    -- session where it can be felt. See helpdocs/CodeReview-2026-09-22.md finding 6.
