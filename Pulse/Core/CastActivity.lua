...
local function keyFor(castGUID, spellID)
    if castGUID then return "g:" .. tostring(castGUID) end
...
function CastActivity:_OnStart(unit, castGUID, spellID)
    if not isPlayer(unit) then return end
    ...
    emit({
        ...
        spellID = spellID, castGUID = castGUID, isCrafting = crafting,
    })
end
...
function CastActivity:_OnChannelStart(unit, castGUID, spellID)
    if not isPlayer(unit) then return end
    self.activeChannel = {
        key = keyFor(castGUID, spellID), spellID = spellID,
        startedAt = GetTime(), succeeded = false,
    }
...
function CastActivity:_OnSucceeded(unit, castGUID, spellID)
    ...
    if self.crafting and self.crafting.spellID == spellID then
        ...
        emit({
            classification = "CRAFT_COMPLETE", spellID = spellID, castGUID = castGUID,
            isCrafting = true, duration = GetTime() - (started or GetTime()),
        })
        return
    end
    ...
    if self.pending[key] then
        ...
        self.pending[key] = nil
        emit({
            classification = "CAST_COMPLETE", spellID = spellID, castGUID = castGUID,
...
function CastActivity:_OnStop(unit, castGUID, spellID)
    ...
    if self.crafting and self.crafting.spellID == spellID then
        ...
        self.crafting = nil
        emit({
            classification = "CRAFT_STOPPED", spellID = spellID, castGUID = castGUID,
            isCrafting = true, duration = GetTime() - (started or GetTime()),
        })
    end
end
...
function CastActivity:_OnChannelStop(unit, castGUID, spellID)
    ...
    emit({
        classification = "CHANNEL_STOP", spellID = spellID, castGUID = castGUID,
...
function CastActivity:_OnCraftBegin(recipeSpellID)
    if type(recipeSpellID) ~= "number" then return end
...
function CastActivity:_OnCraftAborted()
    ...
    self.crafting = nil
    emit({
        classification = "CRAFT_STOPPED", spellID = craft.spellID, isCrafting = true,
...
function CastActivity:_Sweep()
    ...
    for key, cast in pairs(self.pending) do
        if now - (cast.startedAt or now) > STALE_TIMEOUT then self.pending[key] = nil end
    end
...
-- UNIT_SPELLCAST_FAILED_QUIET is deliberately absent. It is the event that produced the
-- dead/ghost auto-attack spam and the Touch of Death Notification spam, and the filter for
-- both lives in Modules/AlertGeneric.lua. Adding it here without bringing that filter
