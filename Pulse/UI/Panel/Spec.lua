...
local MODE_CATEGORIES = {
	-- Snappy & Taps (11)
	TAP = "Snappy & Taps",
	DOUBLE_TAP = "Snappy & Taps",
	TRIPLE_TAP = "Snappy & Taps",
	LONG = "Snappy & Taps",
	STUTTER = "Snappy & Taps",
	RISING = "Snappy & Taps",
	FALLING = "Snappy & Taps",
	...
	THUD = "Heavy Impacts",
	THUMP = "Heavy Impacts",
	DEFLECT = "Heavy Impacts",
	KNOCK = "Heavy Impacts",
	SURGE = "Heavy Impacts",
	PULSE_BEAT = "Heavy Impacts",
	BURST = "Heavy Impacts",
	IMPACT = "Heavy Impacts",
	CRACK = "Heavy Impacts",
	BRAKE = "Heavy Impacts",
...
local function modeOptions()
	...
	for _, modeID in ipairs(Pulse.ModeOrder) do
		...
		options[#options + 1] = {
			value = modeID,
			label = modeID,
			tooltip = mode and mode.label,
			category = MODE_CATEGORIES[modeID] or "Other",
		}
	end
	return options
end
...
function Spec.BuildRootPage()
	...
	rows[#rows + 1] = {
		...
		tooltip = "Displays a shortcut button on the edge of your minimap for quick access to settings and toggling vibration.",
		get = function()
			local cfg = store:Get("minimap")
			return not (cfg and cfg.hide)
		end,
		set = function(value)
			local cfg = store:Get("minimap") or {}
			cfg.hide = not value
			store:Set("minimap", cfg)
			if Pulse.UI.Minimap and Pulse.UI.Minimap.Refresh then
				Pulse.UI.Minimap:Refresh()
			end
		end,
	}
		...
		for _, entry in ipairs(matching) do
			...
			rows[#rows + 1] = {
				...
				tooltip = wantsRhythm and ("%s is felt as %s struck about %.1f times a second, over the bed."):format(
					...
					work.cadence
				) or ("%s has no impacts — it is the bed alone, for the length of the craft."):format(work.label),
				get = function()
