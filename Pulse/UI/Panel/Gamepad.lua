...
local function navTargetOf(row)
    ...
    if row.isHeader or row.isText then return nil end
    if row:IsObjectType("Button") then return row end
    return row.Control
end
...
local function firstNavTarget()
    ...
    if not page or not page.rows then return nil end
...
function Gamepad.FocusContent(button)
    if not DRIVE_BLIZZARD_NAVIGATION then return end
    local nav = smartNav()
    if not nav or not nav.IsInFocusGroup then return end
    if nav:IsInFocusGroup() then return end
    ...
    if nav.activeInfo then
        ...
        if button then nav:SelectButton(button) end
    end
end
...
function Gamepad.UnfocusContent(button)
    ...
    local nav = smartNav()
    if not nav or not nav.IsInFocusGroup then return end
    if not nav:IsInFocusGroup() then return end
...
local function createFooter(debugName, bindings)
    ...
    if not DRIVE_BLIZZARD_NAVIGATION then return false end
    if type(GamepadSharedUtility) ~= "table"
        or type(GamepadSharedUtility.CreatePromptedBindingFooter) ~= "function" then
        return false
    end
    ...
    if not ok or not footer then return false end
...
local function getSliderFooter()
    if sliderFooter ~= nil then return sliderFooter end
    sliderFooter = createFooter("PulsePanelSlider", {
        { key = GAMEPAD_DPAD_LEFT,  func = function() stepSlider(false) end },
        { key = GAMEPAD_DPAD_RIGHT, func = function() stepSlider(true) end },
    })
    return sliderFooter
end
...
function Gamepad.OnSliderSelected()
    ...
    if footer then pcall(footer.ShowAndActivateBindings, footer) end
end

function Gamepad.OnSliderDeselected()
    local footer = getSliderFooter()
    if footer then pcall(footer.HideAndDeactivateBindings, footer) end
end
...
local function openDropdown()
    ...
    if dropdown.MouseDown then dropdown:MouseDown() end
    if dropdown.MouseUp then dropdown:MouseUp() end
end

local function getDropdownFooter()
    if dropdownFooter ~= nil then return dropdownFooter end
    dropdownFooter = createFooter("PulsePanelDropdown", {
        { key = GAMEPAD_DPAD_LEFT,   func = function() stepDropdown(false) end },
        { key = GAMEPAD_DPAD_RIGHT,  func = function() stepDropdown(true) end },
...
local function doRefresh()
    ...
    if not Gamepad.IsEnabled() then return end
...
function Gamepad.Setup()
    ...
    if panel.gamepadSetupDone then return end
    ...
    if panel.SearchBox then Theme.MarkIgnored(panel.SearchBox) end
    ...
    if type(SmartNavigation_AddJumpNavigationOverride) == "function"
        and SMART_NAV_INPUT_DIRECTION and panel.Sidebar then
        for _, button in ipairs(panel.Sidebar.buttons) do
            pcall(SmartNavigation_AddJumpNavigationOverride, button,
                  SMART_NAV_INPUT_DIRECTION.RIGHT, function()
    ...
    if type(SmartNavigation_AddJumpNavigationOverride) == "function"
        and SMART_NAV_INPUT_DIRECTION and panel.CloseButton then
        pcall(SmartNavigation_AddJumpNavigationOverride, panel.CloseButton,
              SMART_NAV_INPUT_DIRECTION.UP, function()
...
function Gamepad.OnShow()
    ...
    if type(GamepadMode) == "table" and GamepadMode.FrameControlsManager then
        pcall(function() GamepadMode.FrameControlsManager:FrameShown(panel) end)
    end
    ...
    if nav and nav.RegisterCallback then
        ...
        pcall(nav.RegisterCallback, nav, "HitLeftEdge",
              function() Gamepad.UnfocusContent() end, panel)
...
function Gamepad.OnHide()
    ...
    if type(GamepadMode) == "table" and GamepadMode.FrameControlsManager then
        pcall(function() GamepadMode.FrameControlsManager:FrameHidden(panel) end)
    end
end
    ...
    if type(InputUtil) == "table" then
        ...
        if type(InputUtil.RegisterGamepadInit) == "function" then
            pcall(InputUtil.RegisterGamepadInit, panel, function()
                if panel:IsShown() then Gamepad.OnShow() end
            end)
        end
    end
end
