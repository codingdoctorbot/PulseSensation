...
local function runTimersTo(untilTime)
    ...
    for _, t in ipairs(timers) do
        if t.at <= untilTime then due[#due + 1] = t else rest[#rest + 1] = t end
    end
    timers = rest
...
C_GamePad = {
    ...
    SetVibration = function() vibrations = vibrations + 1 end,
    StopVibration = function() end,
}

local printed = 0
function print() printed = printed + 1 end
...
Pulse.Database = {
    ...
    GetChannelTuning = function(_, _, _, default) return default end,
    GetChangeEpsilon = function() return 0.0015 end,
...
for _, file in ipairs({
    "Core/Modes.lua", "Core/Devices.lua",
    "Core/Schemas/Standard.lua", "Core/Engine.lua",
}) do
...
Engine.Set = function(self, ...) recreated = recreated + 1 return realSet(self, ...) end
Engine.SetRoles = function(self, ...) recreated = recreated + 1 return realSetRoles(self, ...) end

local realRaw = Engine.RawChannel
local rawCalls = 0
Engine.RawChannel = function(self, ...) rawCalls = rawCalls + 1 return realRaw(self, ...) end
