...
local function buildWindow()
	...
	Theme.StyleActionButton(closeButton, false)
	closeButton:SetScript("OnClick", function()
		Panel.Close()
	end)
	f.CloseButton = closeButton

	-- Status badge at bottom-left: connection dot, cue count, version
	local statusBadge = f:CreateFontString(nil, "ARTWORK", "GameFontDisableSmall")
	statusBadge:SetPoint("BOTTOMLEFT", f, "BOTTOMLEFT", 18, 18)
	...
	local function addonVersion()
		...
		local okMeta, version = pcall(get, ADDON_NAME, "Version")
		if okMeta and type(version) == "string" and version ~= "" then
			return "v" .. version
		end
		return "v0.1.0-alpha"
	end
	...
	local function updateStatusBadge()
		...
		if active then
			f.StatusBadge:SetText(
				("|cff00ff88●|r |cffffffffGamepad Active|r   |cff3a4b5d•|r   |cff8899aa%d Cues|r   |cff3a4b5d•|r   |cff667788%s|r"):format(
					numCues,
					versionStr
				)
			)
			...
			f.StatusBadge:SetText(
				("|cff666666●|r |cff888888Gamepad Inactive|r   |cff3a4b5d•|r   |cff8899aa%d Cues|r   |cff3a4b5d•|r   |cff667788%s|r"):format(
					numCues,
					versionStr
				)
			)
	...
	f.UpdateStatusBadge = updateStatusBadge
	...
	if ok and search then
		...
		local cueCount = (Pulse.Triggers and #Pulse.Triggers) or 189
		if search.Instructions then
			search.Instructions:SetText(("Search %d cues..."):format(cueCount))
		end

		local borderLine = search:CreateTexture(nil, "OVERLAY")
		borderLine:SetHeight(1)
		borderLine:SetPoint("BOTTOMLEFT", search, "BOTTOMLEFT", 2, -2)
		borderLine:SetPoint("BOTTOMRIGHT", search, "BOTTOMRIGHT", -2, -2)
		borderLine:SetColorTexture(Theme.COLOR_ACCENT.r, Theme.COLOR_ACCENT.g, Theme.COLOR_ACCENT.b, 0.8)
		borderLine:Hide()
		search.FocusBorder = borderLine

		search:HookScript("OnEditFocusGained", function()
			if search.FocusBorder then
				search.FocusBorder:Show()
			end
		end)
